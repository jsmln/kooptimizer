...
CREATE OR REPLACE FUNCTION public.sp_get_announcements_by_statuses(p_status announcement_status_enum)
 RETURNS TABLE(announcement_id integer, title character varying, description text, type announcement_type_enum, status_classification announcement_status_enum, scope character varying, sent_at timestamp with time zone, created_at timestamp with time zone, updated_at timestamp with time zone, creator_name character varying, has_attachment boolean, attachment_count integer)
 LANGUAGE plpgsql
AS $function$
BEGIN
    RETURN QUERY
